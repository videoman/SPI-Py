...
static PyObject* transferrgb(PyObject* self, PyObject* arg)
{
	...
	if(!PyArg_ParseTuple(arg, "OOO", &red, &green, &blue))		// "O" - Gets non-NULL borrowed reference to Python argument.
		return NULL;					// As far as I can tell, it's mostly just copying arg[0] into transferTuple
								// and making sure at least one arg has been passed (I think)
	...
	while(i < len)
	{
		...
		tx[p] = (uint8_t)PyInt_AsSsize_t(tempItem);
		tempItem = PyList_GetItem(green, i);		//
		tx[p+1] = (uint8_t)PyInt_AsSsize_t(tempItem);
		tempItem = PyList_GetItem(blue, i);		//
		tx[p+2] = (uint8_t)PyInt_AsSsize_t(tempItem);
		if (!tx[p] && !tx[p+1] && !tx[p+2])
			tx[p] = 1;
		tx[p+3] = 1;
		p += 4;
		i++;
